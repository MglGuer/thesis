...
\hl{Upon the application’s launch, the user is introduced to the scenario selection screen as shown in Figure 4.3. Here, the user may choose a specific conversational context according to their personal development goals, including options for academic, professional, and casual social situations. Then, the chatbot is primed with the necessary classmate, allowing the AI Partner to adopt the relevant persona.}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{figures/newchat_arrow_ui.png}
    \caption{Socius UI showing the "Chats" tab with previous chats and navigational buttons at the bottom.}%Scenario selection and profile tracking are planned features.}
...
\hl{After choosing a scenario the chat interface is loaded, as shown in Figure 4.4. The interface takes after common text messaging applications to provide a familiar and intuitive environment. The goal for this phase is to simulate a safe space for the practice of conversational skills with the application of pragmatic principles, for which the AI Partner will be responsible.}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\textwidth]{figures/chat_ui.png}
    \caption{Socius UI showing the chat window with alternating message colors, timestamps, and input field.}%Scenario selection and profile tracking are planned features.}
    \label{fig:socius_ui}
\end{figure}
\subsection{Personalized Feedback (AI Mentor Phase)}
\hl{The post-conversation feedback phase, or the AI Mentor phase is the final stage. Here, the user is provided with a personalized textual report based on their performance in the previous phase. The AI Mentor serves not only to provide feedback that can be related to Grice’s Maxims, but also present these concepts in a more grounded way, ideally enabling the users to understand concepts without having to sift through much academic jargon. The report will also provide the user with highlights from the previous conversation’s transcript, revealing areas of strength and room for improvement. Finally, the AI-Mentor itself is available for a free-text dialogue, where the user can ask questions regarding the theories, the previous conversation, the user’s understanding, or other points of clarification.}
